...
<div class="title-bar">
    ...
    <h1 class="title-bar-title">
        <span class="d-ib">PICKUPS MANAGEMENT</span>
    </h1>
    <p class="title-bar-description">
        <small  style ="color:#686EE4">PICKUPS REPORTS</small>
                                ...
                                <div class="tab-pane fade active in" id="profile-11">
                                    ...
                                    <div class = "pull-left">
                                        <!-- <button type="button" class="btn btn-primary" style="background-color: #14692E; border-color: #14692E"><i class="icon icon-file-excel-o"></i>&nbsp &nbsp Download As Excel &nbsp &nbsp</button> | -->
                                         <button  class="btn btn-primary" value="Print 1st Div" 
...
<script>
      ...
      var from_date = $('#from_date').val();
            ...
            if(from_date != '' && to_date != '')
            {
                $.ajax({
                    ...
                    url:'<?php echo site_url('reportController/range_summarypickups'); ?>',
                    dataType: 'json',
   ...
   //document.getElementById("daterange").innerHTML = 'DROP OFF REPORT BETWEEN'. concat('from_date').CONCAT('&').concat('to_date');
    ...
    document.getElementById("daterange").innerHTML = 'PICKUPS REPORT BETWEEN   '.concat(mystartdate).concat('  &    ').concat(myenddate);
     document.getElementById("daterangeFORREPORT").innerHTML = 'PICKUPS REPORT BETWEEN   '.concat(mystartdate).concat('  TO      ').concat(myenddate);
                       //  alert(mystartdate);
      ...
      var from_date = $('#from_date').val();
            ...
            if(from_date != '' && to_date != '')
            {
                $.ajax({
                    type:'POST',
                    data:{from_date:from_date, to_date:to_date},
                    url:'<?php echo site_url('reportController/range_summary_for_printingpickups'); ?>',
                    success:function(result)
                    {
